...
function Cold() {
    ...
    }, [userrole, userid, selectedTag, selectedValues,selectedValues2,selectedValues3,date,selectedTag, leadsPerPage, currentPage,searchTerm]);
  ...
  const indexOfLastLead = currentPage * leadsPerPage;
    ...
    const handleUserChange = (selected) => {
        ...
        const selectedValues = selected.map(user => user.value);
        setSelectedValues(selectedValues)
     };
     const handlestatusChange = (selected) => {
        setselectedstatus(selected);
        const selectedValues2 = selected.map(status => status.value);
      ...
      const handlesourceChange = (selected) => {
        setsource(selected);
        const selectedValues3 = selected.map(source => source.value);
        setSelectedValues3(selectedValues3)

     };
      const handletagchage = (selected) => {
        setSelectedTag(selected);
        const selectedValues4 = selected.map(tags => tags.value);
                                    ...
                                    <div className='d-flex justify-content-between mt-3'>
                                        ...
                                        <div className='d-flex justify-content-between'>
                                            {currentLead.Assigned && currentLead.Assigned.Avatar ? (
                                                <div>
                                                    <img src={`${baseUrl}/${currentLead.Assigned.Avatar}`} style={{ width: '35px', height: '35px', borderRadius: '100%' }} width={100} height={100} />
                                                </div>
                                            ) : null}
                                           ...
                                           <div className="text-primary px-2">
                                                <a target="_blank" href={`https://wa.me/${encodeURIComponent(currentLead.Phone)}?text=${encodeURIComponent('Your custom message here')}`}>
