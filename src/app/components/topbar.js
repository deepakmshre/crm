...
const Topbar = ({ sidePanelStat, setSidePanelStat, avtaar, userdata, userrole}) => {
  ...
  return (
    <header className="!w-full h-20 py-1 z-[99] !bg-[#ffffff] flex justify-center fixed shadow-sm top-0">
      <div className=" w-[98%] h-full  flex justify-between items-center px-2">
        <div className="w-1/5">
          ...
          >
            <motion.span
              className="w-10 h-1 bg-black origin-center"
              variants={{
        ...
        <div className="flex items-center justify-end gap-3  w-1/5">
          <Link href={"/profile"} className="flex items-center gap-2 group">
            ...
            <p className="text-black !mb-0  font-Satoshi font-[500]">{username}</p>
            <div className="size-10 bg-slate-300/40 text-black rounded-full overflow-hidden flex justify-center items-center cursor-pointer group-hover:bg-slate-300/70 transition-all duration-300">
              {
                avtaar != null ? <div className=""><Image src={avtaar} width={35} height={20}  /></div>: <FaRegUser className="text-2xl" /> 
              }
            </div>{" "}
          </Link>
          <div
